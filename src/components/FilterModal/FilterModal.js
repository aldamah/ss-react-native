...
class FilterModal extends React.Component {
  ...
  hideModal = () => {
    ...
    const action = { type: "VISIBLE_ACTION", value: false };
  ...
  searchData = async () => {
    ...
    const action = { type: "FILTER_ACTION", value: filteredData };
    that.props.dispatch(action);
  ...
  selectDataDepartment(item) {
    ...
    if (removeDuplicate.length < 1) {
      ...
      const action = { type: "DEPARTMENT_ACTION", value: null };
      this.props.dispatch(action);
      const actionN = {
        type: "DEPARTMENT_NAME_ACTION",
        value: "Select department name",
      };
      this.props.dispatch(actionN);
    } else {
      this.setState(
  ...
  render() {
    ...
    const { TaskReducer, FilterReducer, taskName, intl } = this.props;
        ...
        >
          ...
          >
            ...
            <DropDownPicker
              ...
              arrowColor={COLORS.customYellow}
              containerStyle={{ width: 220 }}
              defaultValue={filter.length < 1 ? null : this.state.departmentId}
              open={true}
...
export default injectIntl(connect(mapStateToProps)(FilterModal));
